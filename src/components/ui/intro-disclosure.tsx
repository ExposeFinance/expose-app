    ...
    >
      ...
      {isCompleted && (
        <motion.div {...fadeInScale} className="absolute right-2 top-2">
          <div className="rounded-full bg-background-inverse p-1">
            <CheckIcon className="w-2 h-2 text-text-inverse" />
          </div>
        </motion.div>
      )}
    </motion.button>
      ...
      <AnimatePresence mode="wait" initial={false}>
        ...
        >
          {!isDesktop && steps[currentStep]?.media && (
            <AspectRatio
              ratio={16 / 9}
              className="lg:overflow-hidden rounded-lg bg-surface-primary "
            >
          ...
          <div className="flex items-center justify-between px-4">
            ...
            <div className="space-x-2">
              {currentStep > 0 && (
                <Button onClick={onPrevious} variant="default">
                  Previous
                </Button>
              )}
              <Button
